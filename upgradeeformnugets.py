...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("Microting.EformBackendConfigurationBase")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microsoft.EntityFrameworkCore.Design'
        ]
        process_repository('Microting.EformBackendConfigurationBase.csproj', packages, 'eform-backendconfiguration-base')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-backendconfiguration-base -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
